            ...
            for each in deployment_status['Deployments']:
                if each['DeploymentId'] == deployment_id:
                    if each['Status'] == 'successful':
                        ...
                        os.system("url=`aws opsworks describe-commands --deployment-id {0} | jq -r '.Commands[0].LogUrl'`; curl -s $url | gunzip".format(deployment_id))
                        return
...
class Rollback(Operation):
    """
    Used to issue a Rollback operation within OpsWorks
    """
    def __init__(self, context):
        self.application_name = None
        self._application_id = None
...
@cli.command(help='Rollback an application')
@click.option('--application', type=click.STRING, required=True, help='OpsWorks Application')
@click.pass_context
